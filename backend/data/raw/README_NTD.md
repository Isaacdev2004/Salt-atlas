...
6. Replace **`data/derived/transit_routes.geojson`** with real `LineString` / `MultiLineString` corridors (e.g. from GTFS `shapes.txt`) when you have route geometry.
7. Re-run `node scripts/loadToPostgres.js` if you use PostgreSQL.
